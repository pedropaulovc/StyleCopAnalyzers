...
namespace StyleCop.Analyzers.LayoutRules
{
    ...
    /// <summary>
    /// Implements a code fix for <see cref="SA1521UseLineEndingingsConsistently"/>.
    /// </summary>
    [ExportCodeFixProvider(LanguageNames.CSharp, Name = nameof(SA1521CodeFixProvider))]
    [Shared]
    internal class SA1521CodeFixProvider : CodeFixProvider
    {
        /// <inheritdoc/>
        public override ImmutableArray<string> FixableDiagnosticIds { get; } =
            ImmutableArray.Create(SA1521UseLineEndingingsConsistently.DiagnosticId);
        ...
        public override Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            foreach (Diagnostic diagnostic in context.Diagnostics)
            {
                context.RegisterCodeFix(
                    CodeAction.Create(
                        LayoutResources.SA1521CodeFix,
                        cancellationToken => GetTransformedDocumentAsync(context.Document, diagnostic, cancellationToken),
                        nameof(SA1521CodeFixProvider)),
                    diagnostic);
            }
